...
public class DepartmentControllerTest {
	...
	public void findAllDepartment_withPagination() throws Exception {
		...
		this.mockMvc.perform(get("/v2/departments").param("page", "0")
				      .param("size", "2"))				   
		              .andExpect(jsonPath("$.departments[0].id", is(equalTo(1))))
                      .andExpect(jsonPath("$.departments[0].name", is("hr")))
                      .andExpect(jsonPath("$.departments[1].id", is(equalTo(2))))
                      .andExpect(jsonPath("$.departments[1].name", is("it")))
                      .andExpect(jsonPath("$.meta.pageNumber", is(0)))
                      .andExpect(jsonPath("$.meta.offset", is(0)))
                      .andExpect(jsonPath("$.meta.pageSize", is(2)))
